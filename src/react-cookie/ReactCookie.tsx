...
// - 쿠키 설정 검색 삭제를 간단한 함수로 작업 할 수 있다.

// 쿠키(cookie)
// 클라이언트가 특정한 웹서비스에 방문하여 사용 중 해당 서비스에서 데이터를 클라이언트 브라우저에 저장하는 기술
// - 이름, 값, 만료일, 도메인, 경로가 포함
...
export default function ReactCookie() {
    ...
    // useCookies : 리엑트에서 쿠키 객체, 쿠키 변경 함수, 쿠키 제거 함수를 배열로 반환하는 react cookie의 훅함수
    ...
    const onAddClickHandler =()=>{
        // setCookie 함수를 사용하여 쿠키를 추가할 수 있다
        // 첫번째 매개변수 : 쿠키 이름
        // 두번째 매개변수 : 쿠키 값
        // 세번째 매개변수(선택) : 옵션 지정
        ...
        setCookie(name,value, {path: '/', expires: new Date()});
    };

    const onShowClickHandler =()=>{
        // cookies 객체에 브라우저가 가지고 있는 쿠키 정보가 담겨있음
        // alert(JSON.stringify(cookies.first));
        // alert(cookies.first);
        alert(cookies[name]);
