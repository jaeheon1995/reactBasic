...
// 조건부 렌더링
// -상황(조건)에 따라서 다르게 렌더링 하려 할때 사용하는 방법
// - if,&&,?: 를 사용하는 방법이 존재
...
// if문을 이용한 조건부 반환
// - jsx외부(return밖)에서 조건에 따라 return을 다르게 주는 방법
function IfComponent({ num }: Props){
    // num값이 양수면 붉은색, 음수면 파란색, 0이면 초록색으로 표시
    if (num > 0){
        return(
...
// 삼항연산자(? :)를 이용한 조건부 렌더링
// jsx 내부(return내부)에서 상황(조건)에따라 다른 렌더링을 하고자 할때
...
//논리연산자를 이용한 조건부 렌더링
// -jsx내부(return내부)에서 상황(조건)에 따라서 렌더링하고자 할 때
...
export default function ConditionmalRender() {
    return (
        <div>
            <IfComponent num={1} />
            <IfComponent num={0} />
